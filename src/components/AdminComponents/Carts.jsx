...
const Carts = () => {
    ...
    const handleCartReject = async (selectedRowId) => {
        try {
            const cartsCollectionRef = collection(db, "purchases");
    ...
    const handleCartApprove = async (selectedRowId) => {
        try {
            const cartsCollectionRef = collection(db, "purchases");
        ...
        <>
            <div className="main w-[500px] lg:w-full flex flex-col justify-center items-center">
                <div className="w-full mb-[50px] pr-2 pl-2 items-start lg:items-center mt-4 sm:mt-8 md:mt-12 lg:mt-16 xl:mt-20 2xl:mt-24 flex flex-col gap-4 sm:gap-6 md:gap-8 lg:gap-10 xl:gap-12 2xl:gap-16 justify-center">
                    <h1 className='font-semibold md:text-[40px] text-[25px] text-white'>Carts</h1>
                                ...
                                renderTopToolbarCustomActions={({ table }) => {
                                    const handleReject = () => {
                                        ...
                                        if (selectedRows.length === 1) {
                                            ...
                                            alert('Rejecting cart with TransactionId: ' + selectedRowId);
                                            handleCartReject(selectedRowId)
                                        } else {
                                    ...
                                    const handleApprove = () => {
                                        ...
                                        if (selectedRows.length === 1) {
                                            const selectedRow = selectedRows[0];
                                            const selectedRowId = selectedRow.original.id;
                                            alert('Approving cart with TransactionId: ' + selectedRowId);
                                            handleCartApprove(selectedRowId)
                                        } else {
